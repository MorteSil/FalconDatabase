...
namespace FalconDatabase.Files
{
    ...
    /// </summary>
    public class AircraftTable : GameFile, IEquatable<AircraftTable>
    {
        #region Properties
        /// <summary>
        /// Collection of <see cref="AircraftDefinition" /> Entries exported from the Database.
        /// </summary>
        public Collection<AircraftDefinition> AircraftDefinitions
        {
            get
            {
                Collection<AircraftDefinition> output = [];
                foreach (DataRow row in dbTable.Rows)
                    output.Add(ToAircraftDefinition(row));
        ...
        /// </summary>
        public DataTable Aircraft { get => dbTable; set => dbTable = value; }
        /// <summary>
        ...
        /// <returns></returns>
        private static AircraftDefinition ToAircraftDefinition(DataRow row)
        {
            try
            {
                AircraftDefinition output = new()
                {
                    CombatClass = int.Parse((string)row[0]),
                    AircraftID = int.Parse((string)row[1]),
                    SignatureID = int.Parse((string)row[2]),
                    SensorType =
                 [
                     int.Parse((string)row[3]),
                    int.Parse((string)row[4]),
                    int.Parse((string)row[5]),
                    int.Parse((string)row[6]),
                    int.Parse((string)row[7]),
                ],
                    SensorIndex =
                 [
                     int.Parse((string)row[8]),
                    int.Parse((string)row[9]),
                    int.Parse((string)row[10]),
                    int.Parse((string)row[11]),
        ...
        public override string ToString()
        {
            ...
            foreach (DataRow row in dbTable.Rows)
            {
                sb.AppendLine("***** Aircraft Table Entry *****");
                sb.Append(ToAircraftDefinition(row).ToString());
            }
        ...
        #region Constructors
        /// <summary>
        /// Default Constructor for the <see cref="AircraftTable"/> object.
        /// </summary>
        public AircraftTable()
        {
            _FileType = GameFileType.DatabaseACD;
            _StreamType = FileStreamType.XML;
            _IsFileModified = false;
            _IsCompressed = false;
        }
        /// <summary>
        /// Initializes an instance of the <see cref="AircraftTable"/> component of the Database with the data contained in the file at <paramref name="filePath"/>.
        /// </summary>
